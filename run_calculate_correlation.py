...
parser.add_argument('--operators',nargs='+',type=str,required=True)
...
T0_ops=[model.get_observable(op)[0] for op in operators]
checkerboard=False
...
for coordss in tqdm(coordsss):
    ...
    print(coordss)
    T,T_op12,logTotal=forward_observable_tensors(T0,T0_ops,coordss,\
